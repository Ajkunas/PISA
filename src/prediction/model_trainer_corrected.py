...
class ModelTrainer:
    def __init__(self, model_type, sequence_creator, epochs=100, lr=0.01, weight_decay=0.01, dropout=0.2, hidden_dim=100, test_size=0.2, task="baseline", evaluate=True, k=5, padding="pre"):
    ...
    def cross_validate(self):
        ...
        for fold, (train_index, test_index) in enumerate(kf.split(self.sequence_creator.world_sequences)):
            ...
            world_seq_train_padded = pad_sequences(world_seq_train, pad_value, max_len_world)
            code_seq_train_padded = pad_sequences(code_seq_train, pad_value, max_len_code)
            error_seq_train_padded = pad_sequences(error_seq_train, pad_value, max_len_error)
            case_seq_train_padded = pad_sequences(case_seq_train, pad_value, max_len_case)

            world_seq_test_padded = pad_sequences(world_seq_test, pad_value, max_len_world)
            code_seq_test_padded = pad_sequences(code_seq_test, pad_value, max_len_code)
            error_seq_test_padded = pad_sequences(error_seq_test, pad_value, max_len_error)
            case_seq_test_padded = pad_sequences(case_seq_test, pad_value, max_len_case)
